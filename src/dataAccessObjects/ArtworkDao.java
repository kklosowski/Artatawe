...
public class ArtworkDao {
    ...
    public artatawe.Artwork getArtwork(int artwork_id) throws SQLException{
    ...
    public List<artatawe.Artwork> getAllArtwork()throws SQLException{
        List<artatawe.Artwork> artworks = new ArrayList<>();
    ...
    private artatawe.Artwork searchArtworkByType(ResultSet rs) throws SQLException{
        if (rs.getString("artwork_type").equals(SCULPTURE)){
            ...
            if (sculptureRs.next()){
                return artatawe.DBUtils.constructSculpture(rs,sculptureRs);
            }
            ...
            if (paintingRs.next()){
                return artatawe.DBUtils.constructPaintingFromRS(rs,paintingRs);
            }
        }
        return null;
    }

    public int updateArtwork(artatawe.Artwork artwork) throws SQLException{
        ...
        if (result>0){
            if (artwork instanceof artatawe.Sculpture){
                artatawe.Sculpture sculpture = (artatawe.Sculpture) artwork;
            ...
            }else if (artwork instanceof artatawe.Painting){
                artatawe.Painting painting = (artatawe.Painting) artwork;
                String updateSculpture = String.format("UPDATE '%s' SET dimension_x = %f,dimension_y = %f " +
    ...
    public int deleteArtwork(artatawe.Artwork artwork) throws SQLException{
        int result = 0;
        if (artwork instanceof artatawe.Sculpture){
    ...
    int insertArtwork(artatawe.Artwork artwork) throws SQLException{
        List<artatawe.Artwork> artworks = getAllArtwork();
        int lastId =0;
        if (artworks.size()>0){
